...
export interface Product {
  id: string;
  name: string;
  category: 'anillos' | 'aros' | 'collares' | 'antiguedades';
  price: number;
  description: string;
  image: string;
  badge?: string;
...
export const initialProducts: Product[] = [
  ...
  },
  {
    ...
    category: 'aros',
    price: 430,
    description: 'Elegantes aros de diseño naturalista que emulan la forma de hojas de olivo con un acabado satinado cepillado.',
...
export function ProductGrid({ onAddToCart, onViewProduct }: ProductGridProps) {
  ...
  return (
    <section id="productos" className="py-20 px-6 bg-[var(--vintage-paper)]">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-[var(--vintage-metal)] max-w-xl mx-auto">
            Explora nuestra colección de joyería hecha a mano y antigüedades llenas de historia. Cada diseño cuenta una historia única.
          </p>
        </div>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {categories.map((category) => (
              <button
                ...
                className={`px-5 py-2.5 rounded text-sm transition-all duration-300 ${
        ...
        <AnimatePresence mode="popLayout">
          {filteredProducts.length === 0 ? (
            ...
            >
              <ShoppingBag className="w-12 h-12 mx-auto mb-4 text-[var(--vintage-metal)] opacity-50" />
            ...
            >
              {filteredProducts.map((product) => (
                <motion.div
                  ...
                  className="group bg-[var(--vintage-cream)] rounded-lg overflow-hidden border border-[var(--vintage-bronze)]/20 flex flex-col justify-between hover:shadow-xl transition-shadow duration-300"
                >
                  {/* Image wrapper */}
                  <div className="relative aspect-square overflow-hidden bg-neutral-100">
                    ...
                    <div className="absolute inset-0 bg-[var(--vintage-dark)]/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
                      {onViewProduct && (
                        <button
                          onClick={() => onViewProduct(product)}
                          className="p-3 bg-[var(--vintage-cream)] text-[var(--vintage-dark)] rounded-full hover:bg-[var(--vintage-gold)] transition-colors duration-300 shadow-lg cursor-pointer"
                          title="Detalles"
                        >
                      ...
                      <button
                        ...
                        className="p-3 bg-[var(--vintage-gold)] text-[var(--vintage-dark)] rounded-full hover:bg-[var(--vintage-bronze)] hover:text-[var(--vintage-cream)] transition-colors duration-300 shadow-lg cursor-pointer"
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-[var(--vintage-bronze)]/10">
                      <span className="text-xl font-semibold text-[var(--vintage-dark)] font-typewriter">
                        ${product.price.toLocaleString('es-CL')}
                      </span>

                      <button
                        onClick={() => onAddToCart(product)}
                        className="px-3.5 py-2 text-xs bg-[var(--vintage-dark)] text-[var(--vintage-cream)] hover:bg-[var(--vintage-gold)] hover:text-[var(--vintage-dark)] rounded transition-all duration-300 flex items-center gap-1.5 font-medium cursor-pointer"
                      >
                        Agregar
                      </button>
                    </div>
